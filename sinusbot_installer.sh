...
if [ "$STATUS" == "200" -a "$DOWNLOAD_URL" != "" ]; then
  greenMessage "Detected latest TS3-Client version as 3.1.10"
else
...
if [ -f CHANGELOG ] && [ $(cat CHANGELOG | awk '/Client Release/{ print $4; exit }') == 3.1.10 ]; then
  ...
  if [[ ! -f TeamSpeak3-Client-linux_amd64-3.1.10.run && ! -f ts3client_linux_$ARCH ]]; then
...
if [ -f TeamSpeak3-Client-linux_amd64-3.1.10.run ]; then
  ...
  su -c "./TeamSpeak3-Client-linux_amd64-3.1.10.run" $SINUSBOTUSER

  cp -R ./TeamSpeak3-Client-linux_$ARCH/* ./
  sleep 2
  rm ./ts3client_runscript.sh
  rm ./TeamSpeak3-Client-linux_amd64-3.1.10.run
